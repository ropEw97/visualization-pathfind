...
void Map::resizeMatrix(int width, int height)
{
    int numRows, numColumns;

    numRows = (height-m_originY) / m_size;
    if ((height-m_originY) % m_size < m_size * 0.5
            && m_originY < m_size * 0.5)
        numRows--;
...
void Map::setNode(unsigned i, unsigned j, const Node& node)
{
...
Node Map::getNode(unsigned i, unsigned j) const
{
    if (i >= m_matrix.size() || j >= m_matrix[0].size())
        return Node();
    return m_matrix[i][j];
...
std::pair<int, int> Map::getIndices(int x, int y) const
{
    int i, j;
    x -= m_originX;
    y -= m_originY;
...
std::vector<Point> Map::getNodePolygon(int xPos, int yPos) const
{
...
std::vector<std::pair<int,int>> Map::getAdjacentIndices(int i,int j)const
{
...
std::vector<Node> Map::getAdjacentNodes(const Node& node) const
{
    ...
    for (unsigned i = 0; i < neighborIndices.size(); i++)
    {
        ...
        if (row < 0 || static_cast<unsigned>(row) >= height()
                || col < 0 || static_cast<unsigned>(col) >= width())
            continue;
